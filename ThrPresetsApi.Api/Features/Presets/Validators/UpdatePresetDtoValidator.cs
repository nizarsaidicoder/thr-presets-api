...
public class UpdatePresetDtoValidator : AbstractValidator<UpdatePresetDto>
{
    public UpdatePresetDtoValidator()
    {
        RuleFor(x => x.Name)
        ...
        RuleFor(x => x.IsPublic)
            .NotNull().WithMessage("Public visibility status is required");

        RuleFor(x => x.TagIds)
            .NotNull().WithMessage("Tag list must be initialized");
    }
}